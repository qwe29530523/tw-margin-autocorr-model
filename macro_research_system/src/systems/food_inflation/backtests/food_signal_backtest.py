...
SIGNAL_SPECS = {
    "food_commodity_momentum": {
        ...
        "source_columns": ["food_commodity_momentum"],
    },
    "grain_pressure": {
        "feature_role": "candidate_signal",
        "source_columns": ["grain_pressure"],
    },
    "meat_protein_pressure": {
        "feature_role": "candidate_signal",
        "source_columns": ["meat_protein_pressure"],
    },
    "food_cpi_trend": {
        "feature_role": "candidate_signal",
        "source_columns": ["food_cpi_trend"],
    },
    "food_ppi_pipeline_pressure": {
        "feature_role": "candidate_signal",
        "source_columns": ["food_ppi_pipeline_pressure"],
    },
    "source_confidence": {
...
TARGET_SPECS = {
    "food_cpi_forward_change": {
        "source_column": "food_cpi",
        "method": "pct_change",
    },
    "headline_cpi_forward_change": {
        "source_column": "headline_cpi",
        "method": "pct_change",
    },
    "breakeven_inflation_forward_change": {
        "source_column": "breakeven_inflation",
        "method": "diff",
    },
    "rates_forward_change": {
        "source_column": "rates_level",
        "method": "diff",
    },
    "risk_asset_proxy_forward_drawdown": {
        "source_column": "risk_asset_proxy",
        "method": "drawdown",
    },
}
    ...
    for signal_name, signal_spec in SIGNAL_SPECS.items():
        ...
        for target_name, target_spec in TARGET_SPECS.items():
            for horizon in horizons:
                target = _build_forward_target(frame, target_spec["source_column"], target_spec["method"], horizon)
                results.append(
                    _evaluate_signal_target(
                        ...
                        target_available=target_spec["source_column"] in frame.columns,
...
def write_food_backtest_summary(summary: list[dict[str, Any]], output_path: str | Path) -> Path:
...
def _build_forward_target(frame: pd.DataFrame, source_column: str, method: str, horizon: int) -> pd.Series:
    ...
    if method == "drawdown":
        return _forward_drawdown(series, horizon)
